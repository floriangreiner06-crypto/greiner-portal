...
def get_pending_approvals():
    ...
    try:
        ...
        # DEBUG: Logging für Team-Ermittlung
        logger.info(f"📋 Pending Approvals: {ldap_username} (employee_id={employee_id})")
        logger.info(f"   Team-Größe: {len(team_members)}")
        if team_members:
            team_names = [f"{m.get('name', '?')} (ID={m.get('employee_id')})" for m in team_members[:5]]
            logger.info(f"   Team-Mitglieder (erste 5): {', '.join(team_names)}")
        
        if not team_members:
            logger.info(f"   ⚠️ Keine Team-Mitglieder gefunden für {ldap_username}")
            return jsonify({
                'success': True,
                'pending': [],
                'count': 0
        ...
        # DEBUG: Logging für zurückgegebene Anträge
        logger.info(f"   ✅ {len(pending)} offene Anträge gefunden")
        if pending:
            pending_names = [f"{p['employee_name']} (ID={p['employee_id']})" for p in pending[:5]]
            logger.info(f"   Anträge (erste 5): {', '.join(pending_names)}")
...
def approve_vacation():
    ...
    try:
        ...
        # DEBUG: Logging für Genehmigungsprozess
        logger.info(f"🔍 Genehmigung: {ldap_username} (employee_id={employee_id}) versucht booking_id={booking_id} zu genehmigen")
        logger.info(f"📋 Team-Größe: {len(team_members)}, Team-IDs: {team_ids}")
        ...
        with db_session() as conn:
            ...
            if not booking:
                logger.warning(f"❌ Buchung nicht gefunden: booking_id={booking_id}")
                return jsonify({'success': False, 'error': 'Buchung nicht gefunden'}), 404

            booking_employee_id = booking[0]
            booking_employee_name = booking[6]
            
            # DEBUG: Logging für Berechtigungsprüfung
            logger.info(f"👤 Buchung gehört zu: {booking_employee_name} (employee_id={booking_employee_id})")
            
            if booking_employee_id not in team_ids:
                logger.warning(f"❌ Genehmigung verweigert: {ldap_username} darf {booking_employee_name} (ID={booking_employee_id}) nicht genehmigen")
                logger.warning(f"   Team-IDs: {team_ids}, Buchung-Employee-ID: {booking_employee_id}")
                logger.warning(f"   Hinweis: Prüfe AD-Manager-Struktur - ist {ldap_username} Manager von {booking_employee_name}?")
                
                return jsonify({
                    'success': False, 
                    'error': f'Keine Berechtigung: Sie können Urlaub von {booking_employee_name} nicht genehmigen. Bitte prüfen Sie die AD-Manager-Struktur oder kontaktieren Sie HR.'
                }), 403

            if booking[1] != 'pending':
                logger.warning(f"⚠️ Buchung hat bereits Status '{booking[1]}': booking_id={booking_id}")
                return jsonify({'success': False, 'error': f'Buchung hat bereits Status: {booking[1]}'}), 400
            
            # DEBUG: Genehmigung erfolgreich
            logger.info(f"✅ Genehmigung erfolgreich: {ldap_username} genehmigt {booking_employee_name} (booking_id={booking_id})")
