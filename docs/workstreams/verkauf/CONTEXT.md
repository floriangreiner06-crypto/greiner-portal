...
- ✅ **Auftragseingang Zielerfüllung:** Monatsziele-API; Summary-Karten mit „Zielerfüllung (Zielplanung): NW/GW IST/Ziel (X%)”; Verkäufer-Tabelle mit Spalten Ziel NW, Ziel GW, Erfüllung % (nur Monatsansicht). Bei Einzelverkäufer-Filter werden individuelle Ziele statt Konzernziele angezeigt.
- ✅ **Jahres-Zielauswertung (2026-04-02):** Neuer API-Endpunkt `/api/verkauf/auftragseingang/jahresuebersicht` liefert pro Monat Ist NW/GW + Ziel NW/GW + kumuliert. Hero-Card (Bootstrap-konsistent) mit Gesamtfortschritt %, NW/GW/Trend-Boxen + Kompakt-Tabelle mit Mini-Fortschrittsbalken pro Monat. Personalisierte Überschriften bei Einzelverkäufer (Vorname). Funktioniert für Kalenderjahr und Geschäftsjahr. Race-Condition-Schutz via Request-ID.
- ✅ **Modellzusammenfassung (2026-04-02):** Verkäufer-Übersicht gruppiert Modelle nach Basismodell statt Varianten einzeln aufzulisten (z.B. “TUCSON FL (5x)” statt 3 einzelne Ausstattungslinien). Kompaktere Darstellung als Inline-Text statt ul-Listen.
