...
def main():
    """Hauptfunktion für AD Department Sync"""
    
    print("=" * 70)
    print("🔐 AD DEPARTMENT SYNC")
    print(f"   Zeitpunkt: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    
    try:
        # LDAP Connector importieren
        ...
        for emp_id, email, first_name, last_name, current_dept, ldap_username in employees:
            ...
            try:
                # AD abfragen - department Attribut direkt
                conn_ldap.search(
        ...
        return {
            ...
            'unchanged': unchanged,
            'not_found': not_found,
            'changes': changes[:10]  # Nur erste 10 für Celery-Result
        }
