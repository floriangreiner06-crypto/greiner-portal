...
ÄNDERUNGEN v1.1:
- Aggressivere Refinanzierungs-Filter
- Santander KOMPLETT (nicht nur mit "Auszahlung")
- "Finanzierung" als Keyword
- "Auszahlung" als Keyword (mit Ausnahmen)
- Mehr Bank-Keywords (TEBA, RCI, FCE, etc.)
"""
...
class UmsatzBereinigung:
    ...
    def kategorisiere_transaktion(self, verwendungszweck):
        ...
        if 'santander' in vz_lower:  # ALLE Santander! (nicht nur mit Auszahlung)
        ...
        # "Einfinanzierung" mit Fahrzeug-Kontext = Refinanzierung
        ...
        # "Auszahlung" ist meist Refinanzierung (außer bei Hersteller-Provisionen)
        # ABER: Erst nach Hersteller-Check!
        ...
        # (Nach Hersteller-Check, damit Provisionen nicht versehentlich gefiltert werden)
    ...
    def berechne_umsatz(self, monat):
        ...
        umsatz_komponenten = {
            ...
            'nicht_kategorisiert': kategorien['nicht_kategorisiert']['summe'],
        ...
        umsatz_bereinigt = (
            ...
            umsatz_komponenten['nicht_kategorisiert']
        )
    ...
    def print_report(self, ergebnis):
        """Drucke Report"""
        print("\n" + "="*80)
        print(f"🎯 GREINER DRIVE - UMSATZ-BEREINIGUNG {ergebnis['monat']} (v1.1)")
        print("="*80)
        ...
        print(f"  Nicht kategorisiert:      {k['nicht_kategorisiert']:>15,.2f} € ✅")
        print(f"  ─────────────────────────────────────────────")
        ...
        if ergebnis['bwa_soll'] > 0:
            ...
            if abs(ergebnis['differenz_prozent']) < 5:
                ...
                print(f"  → 'Nicht kategorisiert' analysieren")
...
def main():
    ...
    print(f"{'='*80}")
    print(f"Version:  1.1 (Aggressive Filters)")
    print(f"Monat:    {monat}")
