#!/usr/bin/env python3
"""
GlobalCube Excel Parser - Gesamtbetrieb
Analysiert Gesamtbetrieb Excel-Datei
...
def parse_gesamtbetrieb_excel(excel_path):
    """Parst Gesamtbetrieb Excel-Datei"""
    print(f"\n{'='*80}")
    print(f"GLOBALCUBE EXCEL - GESAMTBETRIEB")
    print(f"{'='*80}\n")
    ...
    with zipfile.ZipFile(excel_path, 'r') as z:
        ...
        with z.open('xl/worksheets/Sheet1.xml') as f:
            ...
            for row in rows:
                ...
                # Umsatz: "1 - NW" (Summe aller Bereiche)
                ...
                # Einsatz: "2 - GW" (Summe aller Bereiche)
                ...
                # Variable Kosten: "Fertigmachen" + weitere
                ...
                # Indirekte Kosten: Sammle alle Kandidaten
                if isinstance(monat_val, (int, float)) and abs(monat_val) > 10000:
                    if (first_col == 'Kalk. Kosten' or second_col == 'Kalk. Kosten' or
                        ...
                        indirekte_kosten_kandidaten.append({
                            ...
                            'monat': float(monat_val),
                            'ytd': float(ytd_val) if isinstance(ytd_val, (int, float)) else None,
                            'priority': 1 if ('Kalk. Kosten' in (first_col or '') or 'Kalk. Kosten' in (second_col or '')) else 2
                        })
            ...
            # Wähle beste Indirekte Kosten Position
            if indirekte_kosten_kandidaten:
                # Für Gesamtbetrieb: "Kalk. Kosten" hat Priorität
                ...
                print(f"\n✅ Indirekte Kosten ({best['position']}): Monat={best['monat']:,.2f} €, YTD={best['ytd']:,.2f} €")
                print(f"   Alle Kandidaten:")
                for cand in indirekte_kosten_kandidaten:
                    print(f"     - Zeile {cand['row']}: {cand['position']} = {cand['monat']:,.2f} €")
...
def main():
    """Hauptfunktion"""
    print("=" * 80)
    print("GLOBALCUBE EXCEL ANALYSE - GESAMTBETRIEB")
    print("=" * 80)
    ...
    # Parse Excel
    bwa_data = parse_gesamtbetrieb_excel(EXCEL_PATH)
    
    if bwa_data:
        # Vergleiche mit DRIVE
        comparison = compare_with_drive(bwa_data)
        
        # Speichere Ergebnisse
        output_dir = Path("/opt/greiner-portal/docs/globalcube_analysis")
        output_dir.mkdir(exist_ok=True)
        
        json_path = output_dir / "excel_gesamtbetrieb_tag184.json"
        ...
        if comparison:
            comparison_path = output_dir / "excel_gesamtbetrieb_vergleich_tag184.json"
            with open(comparison_path, 'w', encoding='utf-8') as f:
                json.dump(comparison, f, indent=2, ensure_ascii=False)
            print(f"✅ Vergleich gespeichert: {comparison_path}")
