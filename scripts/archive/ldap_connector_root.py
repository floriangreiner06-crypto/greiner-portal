...
class LDAPConnector:
    ...
    def _create_server(self) -> Server:
        ...
        validate_cert = self.config.get('LDAP_VALIDATE_CERT', 'False').lower() == 'true'
        ...
        if use_ssl and not validate_cert:
            ...
            logger.info("⚠️  SSL-Zertifikats-Validierung deaktiviert (Self-Signed Cert)")
        ...
        logger.info(f"✅ LDAP Server konfiguriert: {self.config['LDAP_SERVER']}:{port} (SSL: {use_ssl}, Validate: {validate_cert})")
        return server
