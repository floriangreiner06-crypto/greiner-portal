...
# -*- coding: utf-8 -*-
"""
Hypovereinsbank November-Import für Greiner Portal
==================================================
Importiert November-Daten von Hypovereinsbank KK
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_hypovereinsbank_november.log'),
...
HYPO_DIR = Path("/mnt/buchhaltung/Buchhaltung/Kontoauszüge/Hypovereinsbank/")

class HypoNovemberImporter:
    """Importiert Hypovereinsbank November-Daten"""
    ...
    def find_november_pdfs(self) -> List[Path]:
        ...
        logger.info("🔍 Suche nach Hypovereinsbank November-PDFs...")
        
        patterns = [
            "*2025*11*.pdf",
            "*Nov*2025*.pdf",
            "*11.25*.pdf",
            "*Hypovereinsbank*2025*.pdf"
        ]
        ...
        # Nach Datum filtern (ab 03.11.2025)
        filtered_files = []
        for pdf in pdf_files:
            # Versuche Datum aus Dateinamen zu extrahieren
            date_match = re.search(r'(\d{2})\.(\d{2})\.(\d{2,4})', pdf.name)
            if date_match:
                try:
                    ...
                    # Nur November 2025, ab 04.11.
                    if year == 2025 and month == 11 and day >= 4:
                        filtered_files.append(pdf)
                except:
                    # Wenn Parsing fehlschlägt, nehme Datei mit
                    filtered_files.append(pdf)
            else:
                filtered_files.append(pdf)
        
        # Duplikate entfernen und sortieren
        pdf_files = sorted(set(filtered_files))
    ...
    def get_konto_id(self, iban: str = None) -> Optional[int]:
        """Holt Konto-ID für Hypovereinsbank"""
        self.cursor.execute("""
            SELECT id FROM konten 
            WHERE kontoname LIKE '%Hypovereinsbank%' OR kontoname LIKE '%Hypo%'
            LIMIT 1
        """)
        ...
        logger.error(f"❌ Kein Hypovereinsbank-Konto gefunden")
        return None
    
    def parse_hypo_pdf(self, pdf_path: Path) -> List[Dict]:
        """
        Parst Hypovereinsbank-PDF
        ...
        try:
            ...
            while i < len(lines):
                ...
                # Pattern: DD.MM.YYYY DD.MM.YYYY TEXT BETRAG EUR
                ...
                if match:
                    ...
                    try:
                        ...
                        valutadatum = datetime.strptime(valutadatum_str, '%d.%m.%Y').date()
                    except ValueError:
                        logger.warning(f"⚠️ Ungültiges Datum: {buchungsdatum_str} / {valutadatum_str}")
                        i += 1
                        continue
                    ...
                    while i < len(lines):
                        ...
                        # Füge zu Verwendungszweck hinzu
    ...
    def run(self):
        """Hauptprozess"""
        logger.info("🚀 HYPOVEREINSBANK NOVEMBER-IMPORT")
        logger.info(f"{'='*70}")
        ...
        for pdf_file in pdf_files:
            ...
            # Parse PDF
            transactions = self.parse_hypo_pdf(pdf_file)
...
def main():
    ...
    if not HYPO_DIR.exists():
        logger.error(f"❌ Hypovereinsbank-Verzeichnis nicht gefunden: {HYPO_DIR}")
        sys.exit(1)
    
    try:
        with HypoNovemberImporter(DB_PATH, HYPO_DIR, dry_run) as importer:
            importer.run()
    except Exception as e:
