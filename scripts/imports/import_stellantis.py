...
Version: 2.0 - Angepasst für reale Excel-Struktur
"""
...
try:
    # ZIP-Dateien finden
    zip_files = sorted(glob.glob(f"{STELLANTIS_PATH}/WHSKRELI_*.zip"), 
    ...
    print(f"Gefundene ZIP-Dateien: {len(zip_files)}\n")
            ...
            with zipfile.ZipFile(zip_file, 'r') as zf:
                for name in zf.namelist():
                    if name.endswith('.xlsx'):
                        ...
                        excel_files.append(os.path.join(TEMP_DIR, name))
                        print(f"   ✓ {name}")
    ...
    for excel in excel_files:
        rrdi = os.path.basename(excel).split('_')[1]
        print(f"📋 Verarbeite RRDI: {rrdi}")
        
        # WICHTIG: skiprows=6, Sheet='Vertragsbestand', Spalten 3-15
        df = pd.read_excel(excel, sheet_name='Vertragsbestand', skiprows=6)
