...
def hole_api_daten(datum: date, subsidiary: int = None) -> dict:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            result['kpis']['entgangener_umsatz'] = summary.get('total_verlust_eur', 0)  # Umbenennung!
            ...
            for a in data.get('auftraege', []):
                if a.get('verlust_eur', 0) > 0:
                    result['nachkalkulation'].append({
                        ...
                        'entgangener_umsatz': a.get('verlust_eur', 0),  # Umbenennung!
        ...
        # 3. Tagesbericht (Probleme)
        resp = requests.get(f'{API_BASE}/tagesbericht', params=params, timeout=30)
        if resp.status_code == 200:
            data = resp.json()
            summary = data.get('summary', {})
            
            ohne_vorgabe = summary.get('ohne_vorgabe', 0)
            ueberschritten = summary.get('ueberschritten', 0)
            
            if ohne_vorgabe > 0:
                result['probleme'].append({
                    'icon': '⚠️',
                    'text': f'{ohne_vorgabe} Aufträge ohne AW-Vorgabe',
                    'severity': 'warning'
                })
            ...
            if ueberschritten > 0:
                result['probleme'].append({
                    'icon': '⏰',
                    'text': f'{ueberschritten} Aufträge mit Vorgabe-Überschreitung',
                    'severity': 'warning'
                })
    ...
    <body style="font-family: 'Segoe UI', Arial, sans-serif; color: #333; max-width: 700px; margin: 0 auto; padding: 20px; background: #f5f5f5;">
        ...
        <div style="background: #333; color: #999; padding: 15px; text-align: center; border-radius: 0 0 10px 10px; font-size: 12px;">
            ...
            <p style="margin: 10px 0 0;">
                <a href="https://auto-greiner.de/drive/werkstatt/kompakt" style="color: #4da6ff;">📊 Vollständigen Bericht im Portal ansehen</a>
            </p>
        </div>
