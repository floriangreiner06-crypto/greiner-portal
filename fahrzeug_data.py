...
class FahrzeugData:
    ...
    def get_gw_bestand(
        ...
        with locosoft_session() as conn:
            ...
            # EK = Grundpreis + Zubehoer + Fracht + Einsatzerhoehungen
            # VK = out_sale_price (VK netto)
            # DB = VK - EK (vereinfacht, echte Kalkulation ist komplexer)
            query = f"""
                SELECT
                    ...
                    COALESCE(dv.out_sale_price, 0) as vk_preis,
                    -- Kalk. DB = VK - EK (vereinfacht)
                    ...
                    dv.calc_cost_internal_invoices as kosten_intern,
