    ...
    try:
        ...
        for auftrag_nr, (serviceberater_nr, betrieb) in auftraege_mit_sb.items():
            try:
                ...
                # TAG 194: ML-Daten für Test hinzufügen
                ml_daten = {}
                try:
                    # Hole ML-Daten direkt über Locosoft-DB und ML-Berechnung
                    ...
                    # Hole Auftrag-Daten für ML
                    with locosoft_session() as conn_ml:
                        cursor_ml = conn_ml.cursor(cursor_factory=RealDictCursor)
                        cursor_ml.execute("""
                            SELECT 
                            ...
                            WHERE o.number = %s
                        """, (auftrag_nr,))
                        auftrag_ml = cursor_ml.fetchone()
                        
                        if auftrag_ml and auftrag_ml.get('vorgabe_aw', 0) > 0:
                            # ML-Modell laden
                            ...
                            if os.path.exists(model_path) and os.path.exists(encoder_path):
                                ...
                                # ML-Vorhersage berechnen
                                vorgabe_aw = float(auftrag_ml['vorgabe_aw'])
                                betrieb = auftrag_ml['betrieb'] or 1
                                marke = auftrag_ml['marke'] or 'Opel'
                                km_stand = float(auftrag_ml['km_stand'] or 50000)
                                fahrzeug_alter = float(auftrag_ml['fahrzeug_alter'] or 3)
                                mech_nr = auftrag_ml.get('mechaniker_nr')
                                labour_type = auftrag_ml.get('labour_type') or 'W'
                                
                                auftrag_datum = auftrag_ml['auftrag_datum']
                                if auftrag_datum:
                                ...
                                if model_features and len(model_features) == 9:
                                    features = pd.DataFrame([[
                                        ...
                                        start_stunde, marke_encoded, fahrzeug_alter, km_stand
                                ...
                                # Vorhersage
                                vorhersage_aw = model.predict(features)[0]
                                
                                # Correction Factor
                                correction_factor = 1.0
                                if labour_corrections:
                                    by_type = labour_corrections.get('by_type', {})
                                    correction_factor = by_type.get(labour_type, 1.0)
                                
                                vorhersage_aw_korrigiert = vorhersage_aw * correction_factor
                                potenzial_aw = vorhersage_aw_korrigiert - vorgabe_aw
                                
                                # ML-Status
                                if potenzial_aw > 1.0:
                                    ml_status = 'unterbewertet'
                                    ml_status_icon = '🔴'
                                elif potenzial_aw > 0.3:
                                    ml_status = 'leicht_unterbewertet'
                                    ml_status_icon = '🟡'
                                elif potenzial_aw < -0.5:
                                    ml_status = 'überbewertet'
                                    ml_status_icon = '🟢'
                                else:
                                    ml_status = 'ok'
                                    ml_status_icon = '⚪'
                                ...
                                ml_daten = {
                                    'ml_vorhersage_aw': round(vorhersage_aw_korrigiert, 1),
                                    'ml_potenzial_aw': round(potenzial_aw, 1),
                                    'ml_status': ml_status,
                                    'ml_status_icon': ml_status_icon
                                }
                except Exception as e:
                    logger.debug(f"ML-Daten für Auftrag {auftrag_nr} nicht verfügbar: {e}")
                    ml_daten = {}
                ...
                with db_session() as conn_tracking:
                    ...
                    # TAG 194: ML-Daten hinzufügen (wenn verfügbar)
                    ml_html = ""
                    if ml_daten.get('ml_vorhersage_aw') is not None:
                        ml_vorhersage_aw = ml_daten.get('ml_vorhersage_aw', 0)
                        ml_vorhersage_min = ml_vorhersage_aw * 6
                        ml_potenzial_aw = ml_daten.get('ml_potenzial_aw', 0)
                        ml_status_icon = ml_daten.get('ml_status_icon', '')
                        ml_status = ml_daten.get('ml_status', '')
                        ml_html = f"""
                            <tr style="background: #f0f8ff;">
                                <td style="padding: 10px; border: 1px solid #dee2e6; font-weight: bold;">🤖 ML-Vorhersage</td>
                                <td style="padding: 10px; border: 1px solid #dee2e6;">{ml_vorhersage_aw} AW ({ml_vorhersage_min:.0f} min)</td>
                            </tr>
                            <tr style="background: #f0f8ff;">
                                <td style="padding: 10px; border: 1px solid #dee2e6; font-weight: bold;">📊 ML-Potenzial</td>
                                <td style="padding: 10px; border: 1px solid #dee2e6;">{ml_potenzial_aw:+.1f} AW {ml_status_icon} {ml_status}</td>
                            </tr>
                        """
