...
def lade_vorjahr_gw(
    ...
    try:
        ...
        with db_session() as conn:
            ...
            # BWA-Filter für GW (820000-829999) - SSOT: Nutze zentrale Funktion!
            from api.standort_utils import build_bwa_filter
            
            if standort == 1:
                if nur_stellantis:
                    # Opel DEG: Nur Stellantis (ohne Hyundai)
                    firma_filter_umsatz, firma_filter_einsatz, _, _ = build_bwa_filter('1', '1')
                else:
                    # Deggendorf: Verwende 'deg-both' für beide Subsidiaries
                    firma_filter_umsatz, firma_filter_einsatz, _, _ = build_bwa_filter('0', 'deg-both')
            elif standort == 2:
                # Hyundai: Nur Hyundai
                firma_filter_umsatz, firma_filter_einsatz, _, _ = build_bwa_filter('2', '0')
            elif standort == 3:
                # Landau: Nur Stellantis Landau
                firma_filter_umsatz, firma_filter_einsatz, _, _ = build_bwa_filter('1', '2')
            else:
                # Gesamtbetrieb (alle)
        ...
        with locosoft_session() as conn_loco:
            ...
            # Standort-Filter - SSOT: Nutze zentrale Funktionen!
            from api.standort_utils import build_locosoft_filter_verkauf, build_locosoft_filter_bestand
            standort_filter_verkauf = build_locosoft_filter_verkauf(standort, nur_stellantis)
            standort_filter_bestand = build_locosoft_filter_bestand(standort, nur_stellantis)
