    ...
    try:
        with db_session() as conn:
            ...
            if not row:
                ...
                return _get_approvers_from_rules(cursor, employee_id)
            ...
            if not manager_username:
                ...
                rule_approvers = _get_approvers_from_rules(cursor, employee_id)
...
def _get_approvers_from_rules(cursor, employee_id: int) -> List[Dict]:
    """
    Fallback für Genehmiger-Ermittlung über vacation_approval_rules.
    Nutzt Abteilung + Standort (subsidiary) des Mitarbeiters.
    """
    try:
        cursor.execute("""
            SELECT
                ...
                COALESCE(
                    le.subsidiary,
                    CASE
                        WHEN LOWER(COALESCE(e.location, '')) LIKE '%%landau%%' THEN 3
                        WHEN LOWER(COALESCE(e.location, '')) LIKE '%%hyundai%%' THEN 2
                        WHEN LOWER(COALESCE(e.location, '')) LIKE '%%deggendorf%%' THEN 1
                        ELSE NULL
                    END
                ) AS subsidiary
            FROM employees e
            LEFT JOIN ldap_employee_mapping lem ON lem.employee_id = e.id
            LEFT JOIN loco_employees le
              ON lem.locosoft_id = le.employee_number
             AND le.is_latest_record IS NOT DISTINCT FROM true
            WHERE e.id = %s
            LIMIT 1
        """, (employee_id,))
        ...
        department_name = (emp_row.get('department_name') or '').strip()
        subsidiary = emp_row.get('subsidiary')
        ...
        cursor.execute("""
            SELECT
                ...
                COALESCE(u.display_name, ae.first_name || ' ' || ae.last_name, var.approver_ldap_username) AS approver_name,
                COALESCE(
                    NULLIF(TRIM(u.email), ''),
                    NULLIF(TRIM(u.username), ''),
                    CASE
                        WHEN var.approver_ldap_username IS NOT NULL AND var.approver_ldap_username <> ''
                        THEN var.approver_ldap_username || '@auto-greiner.de'
                        ELSE NULL
                    END
                ) AS approver_email,
                COALESCE(var.approver_ldap_username, alem.ldap_username) AS approver_ldap,
                var.priority,
                var.loco_grp_code,
                var.subsidiary
            FROM vacation_approval_rules var
            LEFT JOIN employees ae ON ae.id = var.approver_employee_id
            LEFT JOIN ldap_employee_mapping alem ON alem.employee_id = var.approver_employee_id
            LEFT JOIN users u
              ON LOWER(TRIM(SPLIT_PART(COALESCE(u.username,'') || '@', '@', 1))) =
                 LOWER(TRIM(SPLIT_PART(COALESCE(var.approver_ldap_username, alem.ldap_username, '') || '@', '@', 1)))
            WHERE var.active = 1
              AND LOWER(TRIM(COALESCE(var.loco_grp_code, ''))) = LOWER(TRIM(%s))
              AND (var.subsidiary IS NULL OR var.subsidiary = %s)
            ORDER BY var.priority ASC, var.id ASC
        """, (department_name, subsidiary))

        approvers = []
        for row in cursor.fetchall():
            approvers.append({
                'approver_id': row.get('approver_employee_id'),
                'approver_name': row.get('approver_name') or 'Unbekannt',
                'approver_ldap': row.get('approver_ldap'),
                'approver_email': row.get('approver_email'),
                'priority': row.get('priority') or 1,
                'ad_group': f"RULE:{row.get('loco_grp_code')}",
                'is_admin': False,
                'is_direct_manager': False
            })
        return approvers
    except Exception as e:
        logger.warning(f"Fallback vacation_approval_rules fehlgeschlagen: {e}")
        return []
    ...
    try:
        with db_session() as conn:
            ...
            def _department_suffixes_from_groups(groups):
                ...
                for g in (groups or []):
                    ...
                    if g.startswith('GRP_Urlaub_Genehmiger_'):
                        ...
                        if not s:
                            continue
                        # Standort-Suffix (z.B. Werkstatt_LAU) normalisieren
                        parts = [p for p in s.split('_') if p]
                        location_hint = None
                        if len(parts) >= 2 and parts[-1].upper() in location_map:
                            location_hint = location_map[parts[-1].upper()]
                            s = "_".join(parts[:-1]).strip()
                        if s:
                            entry = {'suffix': s, 'location_hint': location_hint}
                            if entry not in suffixes:
                        ...
                        if len(parts) >= 2:
                            s = parts[-1].strip()
                            if s:
                                entry = {'suffix': s, 'location_hint': None}
                                if entry not in suffixes:
                                    suffixes.append(entry)
                return suffixes
            ...
            try:
                ...
                for spec in dept_suffixes:
                    suffix = spec.get('suffix')
                    location_hint = spec.get('location_hint')
                    location_filter_sql = ""
                    params = [suffix, suffix]
                    if location_hint:
                        location_filter_sql = " AND LOWER(TRIM(COALESCE(e.location, ''))) LIKE %s"
                        params.append(f"%{location_hint}%")
