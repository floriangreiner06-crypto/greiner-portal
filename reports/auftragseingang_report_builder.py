...
def _build_marken_map(summary_rows: List[Dict[str, Any]]) -> Dict[str, Dict[str, int]]:
    ...
    for row in (summary_rows or []):
        ...
        marken[name]["vfw"] = int(row.get("vfw") or row.get("test_vorfuehr") or 0)
...
def build_auftragseingang_report_package(anchor_date: date) -> Dict[str, Any]:
    ...
    _nw_mm = getattr(VerkaufData, "get_auftragseingang_nw_marke_modell", None)
    if callable(_nw_mm):
        nw_tag = _nw_mm(day=heute_str) or []
        nw_monat = _nw_mm(month=monat, year=jahr) or []
    else:
        nw_tag, nw_monat = [], []
    ...
    if wt["vergangen"] > 0:
        werktage_text += f" · Ø {round(ae_pro_tag, 1)} AE/Tag"
        if prognose_ae is not None:
            werktage_text += f" · Prognose: <strong>{prognose_ae} AE</strong>"
