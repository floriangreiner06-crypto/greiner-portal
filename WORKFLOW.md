...
**NICHT** `{% block extra_js %}` - das existiert nicht!

### Beispiel für neues Template:
```html
{% extends "base.html" %}
...
### Verfügbare Blocks in base.html:
- `{% block title %}` - Seitentitel
- `{% block extra_css %}` - Zusätzliches CSS im `<head>`
- `{% block content %}` - Hauptinhalt
- `{% block scripts %}` - JavaScript am Ende (nach jQuery + Bootstrap)

### Bootstrap & jQuery:
base.html lädt:
1. jQuery 3.7.1
2. Bootstrap 5.3.0 Bundle (inkl. Popper)

Daher kann in `{% block scripts %}` sowohl jQuery (`# 📋 WORKFLOW.md - VERBINDLICHE REGELN FÜR CLAUDE + USER
...
) als auch Bootstrap (`bootstrap.Modal`) verwendet werden.
